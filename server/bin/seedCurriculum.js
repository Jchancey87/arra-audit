...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/arra';
...
async function seed() {
  ...
  try {
    ...
    console.log('Clearing existing 2-week curriculum...');
    await Curriculum.deleteMany({ slug: '2-week-song-audit-planner' });

    console.log('Inserting default 2-week curriculum...');
    const result = await Curriculum.create(curriculumData);
    console.log(`✓ Default curriculum seeded successfully! _id: ${result._id}`);
